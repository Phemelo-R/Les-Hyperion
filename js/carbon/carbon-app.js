...
async function refreshFigures() {
  // Draw immediately from whatever this browser already has
  CarbonCharts.render(CarbonLog.entries);

  const remote = await CarbonLog.pull();

  if (remote && remote.length) {
    // Remote rows are anonymised totals. Local rows for legs this browser
    // saved are dropped from the chart set to avoid double-counting them —
    // they are already in the workbook.
    const unsynced = CarbonLog.entries.filter(e => !e.remote && !CarbonLog.synced.has(e.id));
    CarbonCharts.render(remote.concat(unsynced));
    $c('fig-source').innerHTML = '<span class="sync-dot">●</span>Organisation total · live';
    $c('fig-source').className = 'sync-pill ok';
  } else {
    CarbonCharts.render(CarbonLog.entries.filter(e => !e.remote));
    const msg = CARBON_CONFIG.readEndpoint
      ? 'Pooled figures unavailable — showing this device only'
      : 'This device only — no flow connected';
    $c('fig-source').innerHTML = `<span class="sync-dot">○</span>${msg}`;
    $c('fig-source').className = 'sync-pill';
  }
}
